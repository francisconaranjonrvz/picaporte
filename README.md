...
| ⏳ Pendiente | 2 · Perfil (CV en PDF parseado con LLM) |
...
| [0007](docs/adr/0007-nada-largo-en-una-request.md) | Nada largo en una request: `maxDuration: 15`, workers en Actions |
...
prompts/           prompts versionados de la Anthropic API (fase 4)
docs/adr/          decisiones de arquitectura
.github/workflows/ ci.yml · db.yml
```

Estructura prevista para las fases 2-7 (sin modelos aún, por diseño): `apps/profiles`,
`apps/companies` (Company, SourceRecord, adaptadores `SourceAdapter` → `RawCompany`),
`apps/enrichment`, `apps/tracking` (Favorite, Visit), `apps/routes`, `apps/offers` y
`apps/jobs` (JobRun + disparo de workflows). `apps/core` se mantiene transversal.
...
| `ANTHROPIC_API_KEY`, `GOOGLE_PLACES_API_KEY`, `FOURSQUARE_API_KEY`, `GITHUB_TOKEN` | fases 3-4 | fase 3 (`GITHUB_TOKEN`) | fases 3-4 |
...
Toda respuesta se cachea en la base de datos; recalcular el `fit_score` al cambiar el perfil no
repite ningún fetch.
